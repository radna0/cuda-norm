"""
Benchmark GPT-OSS-120B DFlash speculative decoding across multiple draft checkpoints
in a single GPU job (loads the 120B target once).

This exists to:
  - reduce variance by keeping a single target model resident
  - reduce cost/time by avoiding repeated 120B loads per checkpoint

Usage (example):
  nohup env MODAL_PROFILE=nguyenbinhkien1206 modal run harmony/cuda-norm/modal/dflash_gptoss120b_benchmark_multi.py \
    --ckpt-dirs-csv /root/model/dflash_gptoss120b/<run>/step_000000,/root/model/dflash_gptoss120b/<run>/step_000500 \
    --max-samples 8 --max-new-tokens 2048 --stop-on-eos false \
    > harmony/cuda-norm/unsloth_logs/dflash120b_bench_multi.log 2>&1 &
"""
...
APP_NAME = "dflash-gptoss120b-benchmark-multi"
...
def bench_remote_multi(
    *,
    ...
    for ckpt in ckpt_dirs:
        ckpt_dir_path = Path(str(ckpt))
        if not ckpt_dir_path.exists():
            raise FileNotFoundError(f"ckpt_dir not found: {ckpt_dir_path}")
        draft = (
            GptOssDFlashDraftModel.from_pretrained(str(ckpt_dir_path), torch_dtype=torch.bfloat16)
            .to(target.device)
            .eval()
        )
        ...
        for i, prompt in enumerate(prompts):
            print(f"[ckpt {ckpt_dir_path.name}] [sample {i+1}/{len(prompts)}] start", flush=True)
            ...
            print(
                f"[ckpt {ckpt_dir_path.name}] [sample {i+1}/{len(prompts)}] speedup={results[-1]['speedup']:.3f} "
    ...
    out_dir = Path("/root/model") / "dflash_bench_120b_multi" / run_id
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / "bench_multi.json"
    out_path.write_text(
        json.dumps(
            {
                "run_id": run_id,
                ...
                "ckpt_dirs": [str(x) for x in ckpt_dirs],
                "runs": all_runs,
            },
            indent=2,
        ),
...
def main(
    ...
    ckpt_dirs = [x.strip() for x in (ckpt_dirs_csv or "").split(",") if x.strip()]
    if not ckpt_dirs:
        raise ValueError("--ckpt-dirs-csv must be non-empty")
    predownload_remote.remote(model_id=model_id)
    out = bench_remote_multi.remote(
        ckpt_dirs=ckpt_dirs,
        model_id=model_id,
        max_samples=int(max_samples),
        max_new_tokens=int(max_new_tokens),
